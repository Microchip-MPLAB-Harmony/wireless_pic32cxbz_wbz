...
<#if (BOOTLOADER_DFU_ENABLE == true) && (BOOTLOADER_DFU_MODE == "TIMER_BASED_TRIGGER")>
...
<#if BOOTLOADER_CONSOLE_ENABLE == true>
...
{
  if( 0 == no_printout)
    SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_USART_Write(buf, size);
}
</#if>
...
uint32_t check_signature_ram(uint32_t image_start_address, uint8_t *memory_holder)
{
  ...
  uint8_t       public_key[] =   {${ECC_PUBLIC_KEY}};
  int verifyStatus = 0;
<#if (BOOTLOADER_AUTH_METHOD_SHA256_ENABLE == true) || (BOOTLOADER_AUTH_METHOD_ECDSA256_ENABLE == true)>
  uint8_t *hash_value;
  wc_Sha256 *sha256;
</#if>
...
<#if (BOOTLOADER_AUTH_METHOD_SHA256_ENABLE == true)>
...
<#if (BOOTLOADER_AUTH_METHOD_ECDSA256_ENABLE == true)>
...
<#else>
      /*If failed ECC verification, exit by return*/
      if( ret != 0 )
      {
        return UNAUTH_FW_SIZE;
      }
...
<#if (BOOTLOADER_AUTH_METHOD_NONE_ENABLE == false)>
  /* If requested authentication method is none and Authentication Method None is not supported, then return 0 */
  if( (firmware_header->md_auth_mthd == Auth_none) || (firmware_header->fw_img_auth_mthd == Auth_none))
  {
    return 0;
  }
</#if>
<#if (BOOTLOADER_AUTH_METHOD_SHA256_ENABLE == false)>
  /* If requested authentication method is SHA256 and Authentication Method SHA256 is not supported, then return 0 */
  if( (firmware_header->md_auth_mthd == Auth_sha256) || (firmware_header->fw_img_auth_mthd == Auth_sha256))
  {
    return 0;
  }
</#if>
<#if (BOOTLOADER_AUTH_METHOD_ECDSA256_ENABLE == false)>
  /* If requested authentication method is ECDSA256 and Authentication Method ECDSA256 is not supported, then return 0 */
  if( (firmware_header->md_auth_mthd == Auth_ecdsa256) || (firmware_header->fw_img_auth_mthd == Auth_ecdsa256))
  {
    return 0;
  }
</#if>
...
*@retval None
*/
void SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_USART_deInit(void)
{
  SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_REGS->USART_INT.SERCOM_CTRLA = 0;
  SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_REGS->USART_INT.SERCOM_BAUD = 0;
  SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_REGS->USART_INT.SERCOM_CTRLB = 0;
  
  /* Wait for sync */
  while((SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_REGS->USART_INT.SERCOM_SYNCBUSY) != 0U)
  {
      /* Do nothing */
  }
  
  SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_REGS->USART_INT.SERCOM_INTENCLR = 0xBF;
  SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_REGS->USART_INT.SERCOM_INTFLAG = 0;  
}
</#if>
...
<#if (BOOTLOADER_DFU_ENABLE == true) || (BOOTLOADER_CONSOLE_ENABLE == true)>
  NVIC_DisableIRQ(SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_IRQn);
</#if>
...
<#if (BOOTLOADER_DFU_ENABLE == true) || (BOOTLOADER_CONSOLE_ENABLE == true)>
    SERCOM${BOOTLOADER_DFU_SERCOM_INSTANCE_NUM}_USART_deInit();
</#if>
    NVIC_deInit();
<#if BOOTLOADER_CONSOLE_ENABLE == true>
    for(count = 0; count < 0x5ffff; count++);
...
<#if (BOOTLOADER_DFU_ENABLE == true) && (BOOTLOADER_DFU_MODE == "GPIO_TRIGGER")>
  ...
  portRef = ((gpio_registers_t*)GPIO_${BOOTLOADER_GPIO_PORT})->GPIO_PORT;
  pval = portRef & (1 << ${BOOTLOADER_GPIO_PIN});
...
<#if BOOTLOADER_DFU_ENABLE == true>
<#if BOOTLOADER_DFU_MODE == "GPIO_TRIGGER">
  else
</#if>
  {
    /* Enters DFU Mode */
...
<#else>
  /* Check for image and jump to application */
  check_for_image();
</#if>
  while(1);
}
