...
{
  struct crmhash imgHashCtx;
  int r;
  
  rom_memset (digest, 0x00, 64);    
  rom_memset ((uint8_t *)&imgHashCtx, 0x00, sizeof(struct crmhash)); 
  
  if(hash_sz == 32)
  {
      r = CRM_HASH_CREATE_SHA256(&imgHashCtx, sizeof(struct crmhash));
  } 
  else
  {
      r = CRM_HASH_CREATE_SHA384(&imgHashCtx, sizeof(struct crmhash));
  }
  ...
  r = CRM_HASH_FEED(&imgHashCtx, (const char *)data, len);
  if (r != CRM_OK)
  {
      return false;
  }
  
  r = CRM_HASH_DIGEST((struct crmhash *)&imgHashCtx, (char *) digest);
  ...
  do
  {
      r = CRM_HASH_STATUS(&imgHashCtx);
  } while (r == CRM_ERR_HW_PROCESSING);
...
{
  struct crmblkcipher cipher;
  struct crmkeyref key1;
  int status;
  
  key1 = CRM_KEYREF_LOAD_BY_ID(0);
  status = CRM_BLKCIPHER_CREATE_AESECB_ENC(&cipher, (const struct crmkeyref *)&key1);
  if( status != CRM_OK )
  {
    return false;
  }
  
  status = CRM_BLKCIPHER_CRYPT(&cipher, (const char*)input, size, (char *)output);
  if (status != CRM_OK )
  {
    return false;
  }
  
  status = CRM_BLKCIPHER_RUN(&cipher);
  if (status != CRM_OK )
  {
    return false;
  }

  status = CRM_BLKCIPHER_WAIT(&cipher);
  if (status != CRM_OK )
  {
    return false;
  }
