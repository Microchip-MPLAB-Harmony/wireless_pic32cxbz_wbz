...
#include <string.h>
#include <stdlib.h>
#include "dfu/pe/include/progexec.h"
...
static const IMG_MEM_TOPOLOGY * getDeviceTop(const uint32_t startAddr, 
        ...
        const uint8_t count);
static uint32_t *read_from_scratch(const uint32_t addr, uint8_t * buf, const uint32_t size, 
        IMG_MEM_READ pfRead);
...
//uint8_t pe_command[PE_CMD_SIZE];
uint8_t *pe_command;
int8_t pe_resp[PE_RESP_SIZE+20];
/******************************************************************************
 main
...
int32_t program_exec_main(const IMG_MEM_TOPOLOGY ** tops, uint8_t count)
{
    ...
    pe_command = (uint8_t *)malloc(PE_CMD_SIZE);
    if( pe_command == NULL )
    {
      while(1);
    }
    ...
    {
        /* read command */        
        if (GET_PE_COMMAND(pe_command, PE_CMD_SIZE) <= 0)
            continue;
        ...
        {
            //*( (volatile uint32_t *) PE_RESP_SRAM_ADDR  ) = 
            ...
            *((volatile uint32_t *) pe_resp ) = 
...
#endif 
                //*( (volatile uint32_t *) PE_RESP_SRAM_ADDR + 1 ) = devID;   
            ...
            //*( (volatile uint32_t *)PE_RESP_SRAM_ADDR ) = ( (Cmd.cmd) << 16 ) | 
            //        ( ( retval <= NACK_RESP ) ? retval : FAIL_RESP );
            *( (volatile uint32_t *) pe_resp ) = ( (Cmd.cmd) << 16 ) | 
                    ( ( retval <= NACK_RESP ) ? retval : FAIL_RESP );
        }

        SET_PE_RESPONSE(pe_resp, respSize); 
   } /* end of while */
        ...
        const IMG_MEM_TOPOLOGY *pDeviceTop)
{
    ...
    {
        //rom_memset32((uint32_t *)addr, 0x00000000, len/4);
    }
...
//    uint32_t result = 0;
    ...
    while (len) {
        ...
        read_from_scratch((uint32_t)addr, pBuf, size, 
        ...
        uint32_t * const crc, IMG_MEM_READ pfRead )
{
    ...
    while (len) {
        ...
        if( pBuf == NULL )
          return FAIL_RESP;
        read_from_scratch((uint32_t)addr, pBuf, size, pfRead);                
        ...
        const uint32_t len, IMG_MEM_WRITE pfWrite )
{
    ...
    {
        //rom_memcpy32((uint32_t *)address, (uint32_t *)dataAddr, len/4);
    }
...
* Note:     None
******************************************************************************/
static uint32_t *read_from_scratch(const uint32_t addr, uint8_t *pBuf, const uint32_t size, 
        IMG_MEM_READ pfRead)
{
    if (pfRead)
    {
        memset(pBuf, 0, size);
        //rom_memset32(pBuf, 0, scratch_size/4); 
        if (pfRead(0, (uint8_t *)pBuf, (uint32_t)addr, size))
            return (uint32_t *)0;    
    }
    else
    {
        pBuf = (uint8_t *) addr; /* allowing in-place read */
    }
